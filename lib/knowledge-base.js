...
export function assertKnowledgeBasePathAllowed(filePath, allowedRoot = process.cwd()) {
  ...
  throw new Error(`local_file path must stay within ${resolvedRoot}`);
}

export function updateKnowledgeBaseItem(existingItem, input) {
  const sourceType = validateSourceType(input?.source_type);
  const updated_at = new Date().toISOString();
  const source = normalizeSource(input?.source);
  const item = {
    id: existingItem?.id || input?.id || randomUUID(),
    enabled: input?.enabled !== false,
    updated_at,
    name: requireTrimmedField(input?.name, "name is required"),
    description: trimOptionalString(input?.description),
  ...
  item.content = buildUpdatedContent(existingItem, item, input);
  item.source_summary = deriveSourceSummary(item);
...
function buildUpdatedContent(existingItem, item, input) {
  if (
    item.source_type === "local_file"
    && existingItem?.source_type === "local_file"
    && normalizeSource(existingItem.source).path === item.source.path
  ) {
    return existingItem.content;
  }

  return buildContent(item, input);
}
...
function readFreeText(input) {
  ...
  requireTrimmedField(text, "content.text is required for free_text");
  return text;
}
