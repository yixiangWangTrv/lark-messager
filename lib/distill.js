...
// --- Error parsing helpers for lark-cli JSON output ---
function parseLarkError(result) {
  const err = result.error || {};
  if (err.subtype === "timeout" || err.message?.includes("timeout")) {
    return "Network timeout — check VPN/proxy or try again later";
  }
  return err.message || "Unknown Lark API error";
}

function tryParseLarkError(execErr) {
  // execFile rejects with { stderr, stdout } — try parsing stdout as lark-cli JSON
  const raw = execErr.stdout || execErr.stderr || "";
  try {
    const result = JSON.parse(raw);
    if (result.ok === false) return parseLarkError(result);
  } catch { /* not JSON */ }
  if (execErr.message?.includes("timeout")) return "Network timeout — check VPN/proxy or try again later";
  return null;
}
...
export async function searchContact(name, identity = "user") {
  ...
  try {
    ...
    if (!result.ok) throw new Error(parseLarkError(result));
    ...
    // lark-cli may exit non-zero but still output parseable JSON
    const larkErr = tryParseLarkError(err);
    throw new Error(`Contact search failed: ${larkErr || err.message}`);
  }
}
...
export async function searchGroup(query, identity = "user") {
  ...
  try {
    ...
    const larkErr = tryParseLarkError(err);
    throw new Error(`Group search failed: ${larkErr || err.message}`);
  }
}
...
export async function fetchMessages({ chatId, openId, p2pChatId, limit = 50, identity = "user", source = "p2p" }) {
  ...
  try {
    ...
    const larkErr = tryParseLarkError(err);
    throw new Error(`消息获取失败: ${larkErr || err.message}`);
  }
}
 ...
 * @param {string} options.lang - "zh" or "en", controls analysis and output language
 */
export async function distillPersona({ name, relation, messages, openCodeClient, lang = "zh", source = "p2p" }) {
  // Flexible name matching: case-insensitive, partial match, token match
  ...
  const targetMessages = messages.filter((m) => {
    ...
    // Direct match
    if (sender === nameLower) return true;
    // Partial contains
    if (sender.includes(nameLower) || nameLower.includes(sender)) return true;
    // Token match: any token of the name appears in sender
    if (nameTokens.some((token) => token.length > 1 && sender.includes(token))) return true;
    return false;
  });

  // For p2p chats with 0 matched messages: if we have messages, the non-self messages
  // are likely from the target (since it's a 1-on-1 chat)
  let finalMessages = targetMessages;
  if (targetMessages.length < 5 && source === "p2p" && messages.length > 0) {
    // In p2p, assume messages from the most frequent non-self sender are the target
    const senderCounts = {};
    for (const m of messages) {
      const s = m.sender || "unknown";
      senderCounts[s] = (senderCounts[s] || 0) + 1;
    }
    // Find the sender with most messages (likely the target in p2p)
    const sorted = Object.entries(senderCounts).sort((a, b) => b[1] - a[1]);
    if (sorted.length >= 1) {
      // Use the top sender (or second if top is clearly "self" based on count balance)
      const topSender = sorted[0][0];
      finalMessages = messages.filter((m) => m.sender === topSender);
      // If still looks wrong (top sender matches self), try second
      if (finalMessages.length < 5 && sorted.length >= 2) {
        const secondSender = sorted[1][0];
        finalMessages = messages.filter((m) => m.sender === secondSender);
      }
    }
  }

  if (finalMessages.length < 5) {
    const senderSamples = [...new Set(messages.slice(0, 10).map((m) => m.sender))].join(", ");
    throw new Error(
      `消息数量不足：仅找到 ${finalMessages.length} 条来自"${name}"的消息（最少需要5条）。消息中的发送者名称为: [${senderSamples}]。请增加采集行数或检查名称是否匹配。`
    );
  }

  const relationInfo = RELATIONS.find((r) => r.id === relation) || RELATIONS[0];

  // Build analysis prompt — use English base for id/elv with a language instruction suffix
  const sampleMessages = finalMessages.slice(0, 100).map((m) => `[${m.time}] ${m.sender}: ${m.content}`).join("\n");

  const langSuffix = lang === "id"
    ? `\n\nIMPORTANT: Write the "summary", "style_tags", "personality_traits", and "system_prompt" fields in Bahasa Indonesia.`
    : lang === "elv"
      ? `\n\nIMPORTANT: Write the "summary", "style_tags", "personality_traits", and "system_prompt" fields in Elvish (Sindarin style, Tolkien). Use Sindarin vocabulary and grammar where possible, mixing with English for technical terms.`
      : "";

  const analysisPrompt = lang === "zh"
    ? `你是一个语言风格分析专家。请分析以下聊天记录中"${name}"（身份：${relationInfo.label}）的说话风格和性格特征。
...
请用以下JSON格式输出分析结果（直接输出JSON，不要包裹在代码块中）：
{
  ...
  "system_prompt": "完整的角色扮演提示词，让AI能模仿此人的说话方式。要求：1)使用此人的口头禅和表达习惯 2)模拟其语气和态度 3)保持其回复长度和节奏 4)融入其性格特点。提示词要具体且可执行。"
    ...
    : `You are a language style analysis expert. Analyze the speaking style and personality traits of "${name}" (role: ${relationInfo.label}) from the following chat logs.

Chat logs:
${sampleMessages}

Output the analysis in the following JSON format (output raw JSON, do not wrap in code blocks):
{
  "summary": "One-sentence summary of this person's communication style",
  "style_tags": ["tag1", "tag2", "tag3"],
  "expression_patterns": {
    "tone": "Overall tone description",
    "sentence_length": "short/long/mixed",
    "punctuation_habits": "Punctuation habits",
    "emoji_usage": "Emoji frequency and types",
    "catchphrases": ["phrase1", "phrase2"]
  },
  "personality_traits": ["trait1", "trait2", "trait3"],
  "communication_style": {
    "response_speed": "Response tendency (instant/slow/mood-dependent)",
    "initiative": "Initiative level (proactive/reactive)",
    "conflict_mode": "Reaction pattern in conflicts"
  },
  "system_prompt": "Complete role-play prompt so AI can mimic this person's speaking style. Requirements: 1) Use their catchphrases and expression habits 2) Simulate their tone and attitude 3) Maintain their reply length and rhythm 4) Incorporate their personality traits. The prompt must be specific and actionable."
}${langSuffix}`;
  ...
  if (openCodeClient) {
    try {
      ...
      return localAnalysis(name, relationInfo, finalMessages);
    }
  }
...
export function deleteDistilled(slug) {
  ...
  if (existsSync(filePath)) {
    unlinkSync(filePath);
    // Auto-deactivate if the deleted profile was the active style
    if (_activeStyleSlug === slug) {
      _activeStyleSlug = null;
    }
    return true;
  }
...
/**
 * Delete all distilled profiles and deactivate active style
 */
export function clearAllDistilled() {
  const files = readdirSync(DISTILLED_DIR).filter((f) => f.endsWith(".json"));
  for (const f of files) {
    unlinkSync(resolve(DISTILLED_DIR, f));
  }
  _activeStyleSlug = null;
  return files.length;
}
