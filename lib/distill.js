...
export async function searchContact(name, identity = "user") {
  const args = ["contact", "+search", "--query", name, "--as", identity, "--json"];
  try {
 ...
 */
export async function fetchMessages({ chatId, openId, limit = 50, identity = "user", source = "p2p" }) {
  // If no chatId provided for p2p, try to find it
  let resolvedChatId = chatId;
  if (!resolvedChatId && source === "p2p" && openId) {
    resolvedChatId = await findP2PChatId(openId, identity);
    if (!resolvedChatId) {
