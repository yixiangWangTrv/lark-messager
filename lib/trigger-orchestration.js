import { detectIntent, buildIntentPrompt, buildSessionOptions, parseDistillCommand } from "./intent-router.js";
import {
  getProcessingNotice,
  getThreadOnlyNotice,
  getTriggerOnlyFallbackNotice,
  shouldSendProcessingNotice,
} from "./processing-notice.js";
import { normalizeSessionResult } from "./session-result.js";
import { generateStyledReply, getDistilled, listDistilled, getActiveStylePrompt } from "./distill.js";
import { recordTrigger } from "./recording-service.js";

function shouldRecordTrigger(config, triggerMeta) {
  if (!config.recording?.enabled) return false;
  const mode = triggerMeta?.mode;
  if (!mode) return true;
  return config.recording.modes?.[mode] !== false;
}

export async function processTrigger({
  event,
  config,
  queue,
  triggerGuard,
  contextFetcher,
  opencode,
  replySender,
  triggerMeta = {},
  detectIntentFn = detectIntent,
  buildIntentPromptFn = buildIntentPrompt,
  buildSessionOptionsFn = buildSessionOptions,
  getTodayFn = () => new Date().toISOString().slice(0, 10),
}) {
  const chatId = event.chat_id;
  const messageId = event.message_id;
  const candidateThreadId = event.thread_id || null;
  const isThreadMessage = Boolean(event.thread_id);
  const triggerKey = messageId || `${chatId}:${event.create_time || event.content || "unknown"}`;

  if (triggerGuard && !triggerGuard.tryStart(triggerKey)) {
    return null;
  }

  const queuedTask = queue.enqueue(chatId, async () => {
    let contextResult = null;
    const replyOpts = triggerMeta.replyIdentity ? { identity: triggerMeta.replyIdentity } : {};

    if (shouldSendProcessingNotice(config)) {
      await replySender.sendReply(event, getProcessingNotice(config), { skipPrefix: true, ...replyOpts });

      if (isThreadMessage) {
        await replySender.sendReply(event, getThreadOnlyNotice(config), { skipPrefix: true, ...replyOpts });
      }
    }

    try {
      contextResult = await contextFetcher.fetchContext({
        chatId,
        threadId: candidateThreadId,
        threadLookupMessageId: messageId || null,
        beforeTimestamp: event.create_time,
        triggerMessage: event.content,
      });
    } catch (err) {
      if (shouldSendProcessingNotice(config) && isThreadMessage) {
        await replySender.sendReply(event, getTriggerOnlyFallbackNotice(config), { skipPrefix: true, ...replyOpts });
      }
      throw err;
    }

    const intent = detectIntentFn(event, contextResult.messages, config.intent_routing);

    // Handle distill_style intent directly without normal OpenCode flow
    if (intent === "distill_style") {
      const parsed = parseDistillCommand(event.content);
      if (parsed) {
        // Try to find the distilled profile by slug or name
        const allProfiles = listDistilled();
        const profile = getDistilled(parsed.slug)
          || allProfiles.find((p) => p.name === parsed.slug || p.slug === parsed.slug);

        if (!profile) {
          const available = allProfiles.map((p) => p.name).join(", ") || "none";
          await replySender.sendReply(event, `未找到风格"${parsed.slug}"。可用风格: ${available}`, replyOpts);
          return;
        }

        const userMessage = parsed.message || contextResult.messages?.slice(-1)[0] || "你好";
        const reply = await generateStyledReply({
          slug: profile.slug,
          userMessage,
          openCodeClient: opencode,
        });
        await replySender.sendReply(event, reply, replyOpts);
        return;
      }
    }

    if (shouldSendProcessingNotice(config) && contextResult.scope === "trigger_only" && isThreadMessage) {
      await replySender.sendReply(event, getTriggerOnlyFallbackNotice(config), { skipPrefix: true, ...replyOpts });
    }

    const chatName = await contextFetcher.getChatName(chatId);
    const sessionOptions = buildSessionOptionsFn({
      intent,
      chatId,
      chatName,
      today: getTodayFn(),
      triggerMessageId: messageId,
      triggerContent: event.content,
      threadId: contextResult?.threadId || null,
    });
    const { sessionId, sessionState } = normalizeSessionResult(
      await opencode.findOrCreateSession(sessionOptions, {
        chatName,
        intent,
      })
    );

    const prompt = buildIntentPromptFn({
      intent,
      promptConfig: config.prompt,
      knowledgeBase: config.knowledge_base,
      event,
      contextResult,
      sessionState,
      puaConfig: config.pua,
    });

    // Inject active distill style into prompt if one is set
    const activeStyle = getActiveStylePrompt();
    const finalPrompt = activeStyle?.prompt
      ? `[STYLE OVERRIDE] You must respond in the following style throughout this conversation:\n${activeStyle.prompt}\nApply this tone, vocabulary, and manner to ALL responses. Do not break character.\n\n${prompt}`
      : prompt;

    const analysis = await opencode.sendMessage(sessionId, finalPrompt);
    const replyOptions = triggerMeta.replyIdentity ? { identity: triggerMeta.replyIdentity } : undefined;
    await replySender.sendReply(event, analysis, replyOptions);

    // Record trigger if enabled
    if (shouldRecordTrigger(config, triggerMeta)) {
      try {
        await recordTrigger({
          event,
          config,
          chatName,
          intent,
          analysis,
          threadId: contextResult?.threadId || null,
        });
      } catch (recordErr) {
        process.stderr.write(`[trigger-orchestration] Recording failed: ${recordErr.message}\n`);
      }
    }
  });

  if (queuedTask === null) {
    triggerGuard?.markFailure(triggerKey);
    return null;
  }

  try {
    const result = await queuedTask;
    triggerGuard?.markSuccess(triggerKey);
    return result;
  } catch (err) {
    triggerGuard?.markFailure(triggerKey);
    throw err;
  }
}
