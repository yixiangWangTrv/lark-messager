...
export class ReplySender {
  ...
  async sendReply(event, analysisResult, options = {}) {
    ...
    for (const target of targets) {
      try {
        await this._sendToTarget(target, event, text, identity);
      } catch (err) {
  ...
  async _sendToTarget(target, event, text, identity) {
    if (target === "in_thread") {
      await this._replyInThread(event.message_id, text, identity);
    } else if (target === "in_chat") {
      await this._sendToChat(event.chat_id, text, identity);
    } else if (target.startsWith("dm:")) {
      const userId = target.slice(3);
      await this._sendDm(userId, text, identity);
    } else {
      process.stderr.write(`[reply-sender] Unknown target: ${target}\n`);
    }
  }

  async _replyInThread(messageId, text, identity) {
    const args = [
      "im", "+messages-reply",
  ...
  async _sendToChat(chatId, text, identity) {
    const args = [
      "im", "+messages-send",
      "--chat-id", chatId,
      "--markdown", text,
  ...
  async _sendDm(userId, text, identity) {
    const args = [
      "im", "+messages-send",
      "--user-id", userId,
      "--markdown", text,
