...
async function buildMessageLink(event, identity) {
  ...
  if (resolved) return appendOpenMessageId(resolved, event.message_id);

  const params = new URLSearchParams();
  if (event.thread_id) {
    if (event.chat_id) params.set("open_chat_id", event.chat_id);
    params.set("open_thread_id", event.thread_id);
    if (event.message_id) params.set("open_message_id", event.message_id);
...
function appendOpenMessageId(link, messageId) {
  if (!link || !messageId) return link || "";
  try {
    const url = new URL(link);
    url.searchParams.set("open_message_id", messageId);
    return url.toString();
  } catch {
    return link;
  }
