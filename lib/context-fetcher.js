...
export class ContextFetcher {
  ...
  async fetchContext(input, beforeTimestamp) {
    ...
    if (request.threadId) {
      try {
        const resolvedThreadId = await this._resolveThreadId(request.threadId, request.chatId);
        const messages = await this._fetchThreadMessages(resolvedThreadId, request.beforeTimestamp);
  ...
  async _resolveThreadId(threadId, chatId) {
    ...
    try {
      ...
      const resolved = this._findThreadIdInMessages(messages, threadId);
  ...
  _findThreadIdInMessages(messages, targetMessageId) {
