...
export class ContextFetcher {
  ...
  async fetchContext(input, beforeTimestamp) {
    ...
    if (request.threadId) {
      try {
        const resolvedThreadId = await this._resolveThreadId(request.threadId);
  ...
  async _resolveThreadId(threadId) {
    ...
    const resolvedThreadId = items[0]?.thread_id;

    return resolvedThreadId || threadId;
  }
