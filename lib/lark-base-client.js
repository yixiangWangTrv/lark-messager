...
const RECORD_FIELDS = [
  ...
  { name: "群名", type: "text" },
  { name: "消息总结", type: "text" },
  { name: "消息链接", type: "text", style: { type: "url" } },
...
export async function createBase({ identity, name }) {
  const fields = JSON.stringify(RECORD_FIELDS);
...
export async function ensureRecordingTableSchema({ identity, appToken, tableId }) {
  const existingFields = await getTableFields({ identity, appToken, tableId });
  const fieldNames = new Set(existingFields.map((field) => field.name));

  if (!fieldNames.has("创建时间")) {
    await execFileAsync("lark-cli", [
      "base", "+field-create",
      "--base-token", appToken,
      "--table-id", tableId,
      "--json", JSON.stringify({ name: "创建时间", type: "datetime" }),
      "--as", identity,
      "--format", "json",
    ], { timeout: 30000 });
  }

  if (!fieldNames.has("消息链接")) {
    await execFileAsync("lark-cli", [
      "base", "+field-create",
      "--base-token", appToken,
      "--table-id", tableId,
      "--json", JSON.stringify({ name: "消息链接", type: "text", style: { type: "url" } }),
      "--as", identity,
      "--format", "json",
    ], { timeout: 30000 });
  } else {
  ...
  if (!fieldNames.has("Message ID")) {
    await execFileAsync("lark-cli", [
      "base", "+field-create",
      "--base-token", appToken,
      "--table-id", tableId,
      "--json", JSON.stringify({ name: "Message ID", type: "text" }),
      "--as", identity,
      "--format", "json",
    ], { timeout: 30000 });
  }

  if (fieldNames.has("状态")) {
    await execFileAsync("lark-cli", [
      "base", "+field-update",
      "--base-token", appToken,
      "--table-id", tableId,
      "--field-id", "状态",
      "--json", JSON.stringify({ name: "状态", type: "select", multiple: false, options: STATUS_OPTIONS }),
      "--as", identity,
      "--format", "json",
      "--yes",
    ], { timeout: 30000 });
