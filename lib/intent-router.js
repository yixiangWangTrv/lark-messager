...
export function buildIntentPrompt(intentOrOptions, promptConfig, event, contextLines = []) {
  ...
  const operationalSafetyInstruction = resolvedPromptConfig?.operational_safety?.instruction || DEFAULT_OPERATIONAL_SAFETY_INSTRUCTION;
  const sections = [];
  ...
  if (sessionState === "new") {
    ...
    if (operationalSafetyInstruction) {
      sections.push(operationalSafetyInstruction);
    }
