...
export class OpenCodeClient {
  ...
  async findOrCreateSession(chatId, chatName, today = new Date().toISOString().slice(0, 10)) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const match = sessions.find((s) => {
          const sessionDirectory = s.cwd || s.directory || s.projectDirectory || null;
          return s.title === targetTitle && (!sessionDirectory || sessionDirectory === this.projectDirectory);
        });
        if (match) {
