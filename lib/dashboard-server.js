...
import { createBase, createTable, ensureRecordingTableSchema, getBase, parseBaseUrl } from "./lark-base-client.js";
...
export class DashboardServer {
  constructor({
    config, botEvents, configPath, opencode, todoStorePath,
    botLockPath, processExists, killProcess, spawnBotProcess, getCurrentProcessPid,
  }) {
    this._config = config;
    ...
    this._processExists = processExists ?? ((pid) => {
      try { process.kill(pid, 0); return true; } catch (err) { return err?.code !== "ESRCH"; }
    });
    this._killProcess = killProcess ?? ((pid, signal) => process.kill(pid, signal));
    this._spawnBotProcess = spawnBotProcess ?? (({ execPath, argv, cwd }) => spawn(execPath, argv, { cwd, detached: true, stdio: "ignore" }));
    this._getCurrentProcessPid = getCurrentProcessPid ?? (() => process.pid);
    this._sessions = [];
  ...
  _registerSession(session) {
    ...
    this._broadcast({ eventName: "session_created", data: session });
  }
  ...
  async start() {
    ...
    return new Promise((resolve_p, reject) => {
      const onListening = () => { this._httpServer?.removeListener("error", onError); resolve_p(); };
      const onError = (error) => { this._httpServer?.removeListener("listening", onListening); this._httpServer = null; reject(error); };
  ...
  _handleRequest(req, res) {
    ...
    if (path === "/api/recording" && req.method === "GET") {
      return this._jsonResponse(res, this._config.recording || { enabled: false, lark_base: { url: "", app_token: "", table_id: "", table_name: "Oncall Bot Records" } });
    }
    if (path === "/api/recording" && req.method === "PUT") {
      return this._handlePutRecording(req, res);
    }
    if (path === "/api/recording/init-table" && req.method === "POST") {
      return this._handleInitRecordingTable(req, res);
    }
  ...
  async _handlePutConfig(req, res) {
    try {
      ...
      for (const [key, val] of Object.entries(body)) {
        if (isPlainObject(val) && isPlainObject(this._config[key])) {
          deepMergeObjects(this._config[key], val);
        } else {
          this._config[key] = val;
        }
      }
      if (body.opencode?.project_directory && this._opencode?.updateConnection) {
        this._opencode.updateConnection({ projectDirectory: this._config.opencode.project_directory });
      }
      if (body.opencode?.project_directory && this._openCodeClient?.updateConnection && this._openCodeClient !== this._opencode) {
        this._openCodeClient.updateConnection({ projectDirectory: this._config.opencode.project_directory });
      }
      this._saveConfig();
  ...
  async _handlePutPrompts(req, res) {
    try {
      const body = await this._readBody(req);
      deepMergeObjects(this._config.prompt, body);
      this._saveConfig();
  ...
  async _handlePutTriggerModes(req, res) {
    try {
      ...
      if (typeof body.allow_private === "boolean") modes.allow_private = body.allow_private;
      if (typeof body.mention_owner_reply_identity === "string") modes.mention_owner_reply_identity = body.mention_owner_reply_identity;
      if (body.mention_owner_group_filter && typeof body.mention_owner_group_filter === "object") {
        modes.mention_owner_group_filter = {
          mode: body.mention_owner_group_filter.mode || "none",
          group_ids: Array.isArray(body.mention_owner_group_filter.group_ids) ? body.mention_owner_group_filter.group_ids : [],
        };
      }
      this._saveConfig();
  ...
  async _handlePutRecording(req, res) {
    try {
      const body = await this._readBody(req);
      if (!this._config.recording) {
        this._config.recording = { enabled: false, modes: {}, lark_base: { url: "", app_token: "", table_id: "", table_name: "Oncall Bot Records" } };
      }
      if (typeof body.enabled === "boolean") this._config.recording.enabled = body.enabled;
      if (body.modes && typeof body.modes === "object") {
        this._config.recording.modes = {
          ...this._config.recording.modes,
          mention_bot: body.modes.mention_bot !== false,
          mention_owner: body.modes.mention_owner !== false,
          all_messages: body.modes.all_messages !== false,
          allow_private: body.modes.allow_private !== false,
        };
      }
      if (body.lark_base && typeof body.lark_base === "object") {
        const parsed = parseBaseUrl(body.lark_base.url);
        this._config.recording.lark_base = {
          ...this._config.recording.lark_base,
          ...body.lark_base,
          app_token: parsed.appToken || body.lark_base.app_token || this._config.recording.lark_base.app_token || "",
          table_id: parsed.tableId || body.lark_base.table_id || this._config.recording.lark_base.table_id || "",
        };
      }
      this._saveConfig();
      this._jsonResponse(res, this._config.recording);
    } catch (e) {
      res.writeHead(500, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ error: e.message }));
    }
  ...
  async _handleInitRecordingTable(req, res) {
    try {
      const identity = this._config.lark.reply_identity || "user";
      const tableName = this._config.recording?.lark_base?.table_name || "Oncall Bot Records";

      const { appToken, tableId: createdTableId, url } = await createBase({ identity, name: tableName });
      const tableId = createdTableId || await createTable({ identity, appToken, tableName: "Records" });
      await ensureRecordingTableSchema({ identity, appToken, tableId });

      if (!this._config.recording) {
        this._config.recording = { enabled: false, modes: {}, lark_base: { url: "", app_token: "", table_id: "", table_name: "Oncall Bot Records" } };
      }
      this._config.recording.lark_base.app_token = appToken;
      this._config.recording.lark_base.table_id = tableId;
      const baseUrl = url || (await getBase({ identity, appToken })).url || "";
      this._config.recording.lark_base.url = baseUrl ? `${baseUrl}?table=${tableId}` : "";
      this._config.recording.enabled = true;
      this._saveConfig();

      this._jsonResponse(res, { ok: true, url: this._config.recording.lark_base.url, app_token: appToken, table_id: tableId });
    } catch (e) {
      res.writeHead(500, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ error: e.message }));
    }
  ...
  async _handleGetServers(res) {
    ...
    if (botEntry) merged.unshift(botEntry);
  ...
  _getBotProcessEntry() {
    return getActiveBotProcess({ lockPath: this._botLockPath, processExistsFn: this._processExists });
  }

  _getBotProcessById(id) {
    const botEntry = this._getBotProcessEntry();
    if (!botEntry || botEntry.id !== id || botEntry.pid !== this._getCurrentProcessPid()) return null;
    return botEntry;
  }

  _stopBotProcess(id, res) {
    const bot = this._getBotProcessById(id);
    if (!bot) { res.writeHead(404, { "Content-Type": "application/json" }); res.end(JSON.stringify({ error: "server not found" })); return true; }
    let done = false;
    const terminate = () => { if (done) return; done = true; try { this._killProcess(bot.pid, "SIGTERM"); } catch {} };
    res.once("finish", terminate); res.once("close", terminate);
  ...
  _restartBotProcess(id, res) {
    const bot = this._getBotProcessById(id);
    if (!bot) { res.writeHead(404, { "Content-Type": "application/json" }); res.end(JSON.stringify({ error: "server not found" })); return true; }
    if (!bot.execPath || !bot.projectDir || !Array.isArray(bot.argv)) { res.writeHead(500, { "Content-Type": "application/json" }); res.end(JSON.stringify({ error: "bot process metadata missing from lock file" })); return true; }
    const handoff = this._spawnBotProcess({ execPath: process.execPath, argv: ["-e", this._buildRestartScript(bot)], cwd: bot.projectDir });
    if (typeof handoff?.unref === "function") handoff.unref();
    let done = false;
    const terminate = () => { if (done) return; done = true; try { this._killProcess(bot.pid, "SIGTERM"); } catch {} };
    res.once("finish", terminate); res.once("close", terminate);
    this._jsonResponse(res, { ok: true, replacementPid: handoff?.pid ?? null });
    return true;
  }

  _buildRestartScript(bot) {
    return [
      'const {readFileSync}=require("node:fs");const {spawn}=require("node:child_process");',
      `const oldPid=${JSON.stringify(bot.pid)};const oldStartedAt=${JSON.stringify(bot.startedAt)};`,
      `const lockPath=${JSON.stringify(this._botLockPath)};`,
      `const execPath=${JSON.stringify(bot.execPath)};const argv=${JSON.stringify(bot.argv)};const cwd=${JSON.stringify(bot.projectDir)};`,
      "const owned=()=>{try{const p=JSON.parse(readFileSync(lockPath,'utf-8'));return p&&p.pid===oldPid&&p.startedAt===oldStartedAt;}catch{return false;}};",
      "const launch=()=>{const c=spawn(execPath,argv,{cwd,detached:true,stdio:'ignore'});if(c.unref)c.unref();};",
      "const wait=()=>{if(!owned()){launch();return;}try{process.kill(oldPid,0);setTimeout(wait,250);}catch(e){if(e&&e.code!=='ESRCH')throw e;launch();}};wait();",
    ].join("");
  }
  ...
  async _handleDistillStart(req, res) {
    try {
      ...
      const respFmt = this._config.prompt?.common_task?.response_format || "";
      let cfgLang = "en";
